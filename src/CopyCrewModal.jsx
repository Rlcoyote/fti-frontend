...
function CopyCrewModal({ jobId, excludeTicketId, existingCrewUserIds, onClose, onCopy }) {
  ...
  useEffect(() => {
    if (!jobId) return;
    setLoadingRigUps(true); setError("");
  ...
  useEffect(() => {
    if (!sourceId) { setSourceCrew([]); return; }
    setLoadingCrew(true); setError("");
  ...
  const handleSubmit = async () => {
    if (willCopy.length === 0) { onClose(); return; }
    setSubmitting(true);
    try {
      await onCopy(willCopy.map(c => ({
        user_id: c.user_id,
      ...
      >
          ...
          <>
                  ...
                  >
                    {rigUps.map(t => (
                      <option key={t.id} value={t.id}>
                        Rig Up #{t.jobId}{t.ticketNumber ? `-${t.ticketNumber}` : ""} · {String(t.date || "").slice(0, 10)}
                      </option>
                    ))}
                  </select>
                  <button
                ...
                {sourceCrew.map((c, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {dup && (
                        <span style={{ fontSize: 10, color: C.muted, fontStyle: "italic" }}>already on crew · skipped</span>
                      )}
                    </div>
                  );
                })}
            ...
            {skipped.length > 0 && (
              <div style={{
                ...
                background: "#fdf5d8", border: `1px solid #8a650044`, borderRadius: 4,
                fontSize: 11, color: "#8a6500", fontWeight: 700,
              }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Btn
            ...
            disabled={submitting || loadingCrew || willCopy.length === 0}
          >
