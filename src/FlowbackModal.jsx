        ...
        if (data && Array.isArray(data) && data.length > 0) {
          const loaded = HOURS.map((hr) => {
            ...
            if (match) {
              return {
                choke: match.choke || "",
                flPsi: match.fl_psi != null ? String(match.fl_psi) : "",
                tbgPsi: match.tbg_psi != null ? String(match.tbg_psi) : "",
                csgPsi: match.csg_psi != null ? String(match.csg_psi) : "",
                temp: match.temp != null ? String(match.temp) : "",
                h2oHr: match.h2o_hr != null ? String(match.h2o_hr) : "",
                oilHr: match.oil_hr != null ? String(match.oil_hr) : "",
                gasHr: match.gas_hr != null ? String(match.gas_hr) : "",
  ...
  const saveDay = () => {
    ...
    const payload = {
      rows: HOURS.map((hr, idx) => ({
        hour: hr,
        choke: rows[idx].choke || null,
        fl_psi: rows[idx].flPsi || null,
        tbg_psi: rows[idx].tbgPsi || null,
        csg_psi: rows[idx].csgPsi || null,
        temp: rows[idx].temp || null,
        h2o_hr: rows[idx].h2oHr || null,
        oil_hr: rows[idx].oilHr || null,
        gas_hr: rows[idx].gasHr || null,
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "16px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: C.muted }}>
              Work Order #{job.id} — {job.customer} · {job.wells[0]}
            </div>
          </div>
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: C.muted }}>DAY:</span>
            <button
              onClick={() => changeDay(Math.max(1, dayNum - 1))}
              style={{ background: C.steel, border: `1px solid ${C.border}`, borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontWeight: 700 }}
            >
              ◀
            </button>
            <span style={{ fontSize: 16, fontWeight: 800, minWidth: 30, textAlign: "center" }}>{dayNum}</span>
            <button
              onClick={() => changeDay(Math.min(31, dayNum + 1))}
              style={{ background: C.steel, border: `1px solid ${C.border}`, borderRadius: 4, padding: "4px 10px", cursor: "pointer", fontWeight: 700 }}
            >
              ▶
            </button>
        ...
        <div style={{ display: "flex", gap: 16, padding: "10px 24px", background: C.steel, borderBottom: `1px solid ${C.border}` }}>
          <div>
            <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>H2O TODAY: </span>
            <span style={{ fontSize: 13, fontWeight: 800, color: C.blue }}>{totalH2O}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.muted, marginLeft: 3 }}>bbls</span>
          </div>
          <div>
            <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>OIL TODAY: </span>
            <span style={{ fontSize: 13, fontWeight: 800, color: C.green }}>{totalOil}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.muted, marginLeft: 3 }}>bbls</span>
          </div>
          <div>
            <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>GAS TODAY: </span>
            <span style={{ fontSize: 13, fontWeight: 800, color: C.orange }}>{totalGas}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.muted, marginLeft: 3 }}>MCF</span>
          </div>
          <div>
            <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>TOTAL FLUID: </span>
            <span style={{ fontSize: 13, fontWeight: 800 }}>{totalH2O + totalOil}</span>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.muted, marginLeft: 3 }}>bbls</span>
          </div>
          {totalH2O + totalOil > 0 && (
            <div>
              <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>OIL CUT: </span>
              <span style={{ fontSize: 13, fontWeight: 800, color: C.red }}>{((totalOil / (totalH2O + totalOil)) * 100).toFixed(1)}%</span>
            </div>
          )}
        </div>
                    ...
                    <td key={f} style={{ padding: "2px 2px" }}>
                      <input
                        style={{
                          ...
                          padding: "3px 4px",
                          fontSize: 11,
                          textAlign: f === "remarks" ? "left" : "right",
                          width: f === "remarks" ? 120 : 55,
                        }}
                        value={rows[idx][f]}
        ...
        <div style={{ padding: "12px 24px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {saveMsg && (
            <span style={{ fontSize: 12, fontWeight: 700, color: saveMsg === "Saved" ? C.green : C.red, marginLeft: 8 }}>
              {saveMsg === "Saved" ? "✓ " : "✗ "}
              {saveMsg}
            </span>
          )}
