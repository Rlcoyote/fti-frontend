...
function SafetyPage() {
  ...
  const fetchCerts = async () => {
    try {
      ...
      console.error("Fetch certs failed:", err);
    }
    setLoading(false);
  };
  ...
  const openEdit = (c) => {
    ...
    setFormCertNum(c.cert_number || "");
    setFormNotes(c.notes || "");
    setFormPhoto(c.photo_url || null);
    setEditCert(c);
    setShowAdd(true);
  };

  const handleSave = async () => {
    if (!formUserId || !formType || !formName) {
      setMsg("Employee, type, and name are required.");
      return;
    }
    const payload = {
      user_id: formUserId,
  ...
  return (
    <div style={{ padding: isMob ? "16px 12px" : "24px 28px" }}>
      ...
      {msg && (
        <div style={{ fontSize: 12, fontWeight: 700, color: msg.includes("fail") || msg.includes("Error") ? C.red : C.green, marginBottom: 12 }}>{msg}</div>
      )}
                  ...
                  {filtered.map((c, i) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          display: "grid",
                          gridTemplateColumns: "1fr 120px 1fr 120px 120px 80px 80px",
                          padding: "10px 14px",
                      ...
                      >
                        ...
                        <div style={{ fontSize: 12, color: C.text }}>
                          ...
                          {c.photo_url && (
                            <span style={{ color: C.green, marginLeft: 6, fontSize: 10 }} title="Photo attached">
                              📷
                            </span>
                          )}
                        </div>
        ...
        >
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div>
                <input type="file" accept="image/*" capture="environment" onChange={handlePhotoSelect} style={{ fontSize: 11, color: C.text }} />
      ...
      {showImport && (
        <ModalWrap
          title="IMPORT CERTIFICATIONS FROM EXCEL"
          onClose={() => {
            setShowImport(false);
          ...
          {!importPreview ? (
            <>
              <div style={{ fontSize: 13, color: C.muted, marginBottom: 16, lineHeight: 1.6 }}>
                Upload an Excel file using the standard template. Employee names must exactly match users in the system. Download the template first if you
                don't have it.
              </div>
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>SELECT EXCEL FILE (.xlsx)</label>
                <input type="file" accept=".xlsx,.xls" onChange={(e) => setImportFile(e.target.files[0])} style={{ fontSize: 13, color: C.text }} />
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <Btn
            ...
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                >
                  {importing ? "IMPORTING..." : `IMPORT ${importPreview.valid} CERTIFICATIONS`}
                </Btn>
                <Btn
                  variant="ghost"
                  onClick={() => {
