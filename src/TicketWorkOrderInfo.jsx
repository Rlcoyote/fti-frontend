...
function TicketWorkOrderInfo({ job, assignedWells }) {
  if (!job) return null;

  const wellsToShow = assignedWells?.length > 0 ? assignedWells : (job.wells || []).map((w) => w.well_name || w);
  ...
  return (
    <div style={{ background: C.steel, borderBottom: `1px solid ${C.border}`, padding: "12px 24px" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 24px", fontSize: 12 }}>
          ...
          <span>
            ...
            {partialWells && (
              <span style={{ color: C.muted, fontSize: 10 }}>
                {" "}
                ({assignedWells.length} of {job.wells.length})
              </span>
            )}
          </span>
        )}
        {job.afe && (
          <span>
            <span style={{ color: C.muted }}>AFE: </span>
            <strong>{job.afe}</strong>
          </span>
        )}
        {job.companyCode && (
          <span>
            <span style={{ color: C.muted }}>Co. Code: </span>
            <strong>{job.companyCode}</strong>
          </span>
        )}
        {job.costCenter && (
          <span>
            <span style={{ color: C.muted }}>Cost Center: </span>
            <strong>{job.costCenter}</strong>
          </span>
        )}
        {job.po && (
          <span>
            <span style={{ color: C.muted }}>PO: </span>
            <strong>{job.po}</strong>
          </span>
        )}
...
export default TicketWorkOrderInfo;
