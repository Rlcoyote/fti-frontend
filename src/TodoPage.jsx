...
function TodoPage({ todos, setTodos, jobs, onNavigateJob, userNames, userIdByName }) {
  ...
  const handleSave = async (form) => {
    const payload = {
      ...
      created_by: userIdByName[getCurrentUser()], assigned_to: userIdByName[form.assignedTo] || userIdByName[getCurrentUser()],
    };
    try {
      const r = await fetch(`${API_URL}/todos`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (r.ok) {
        const saved = await r.json();
        const newTodo = { id: saved.id, ...form, createdBy: getCurrentUser(), assignedTo: form.assignedTo, completed: false, completedBy: null, completedAt: null };
...
function JobTodoTab({ jobId, todos, setTodos, jobs, userNames, userIdByName }) {
  ...
  const handleSave = async (form) => {
    ...
    try {
      ...
      if (r.ok) {
        const saved = await r.json();
        setTodos(prev => [{ id: saved.id, ...form, createdBy: getCurrentUser(), assignedTo: form.assignedTo, completed: false, completedBy: null, completedAt: null }, ...prev]);
      }
    } catch (err) { console.error("Todo create failed:", err); }
    setShowForm(false);
  };
