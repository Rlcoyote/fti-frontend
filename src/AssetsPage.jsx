...
function AssetsPage({ jobs }) {
  ...
  useEffect(() => { const h = () => setPgW(window.innerWidth); window.addEventListener("resize", h); return () => window.removeEventListener("resize", h); }, []);
  const isMob = pgW < 900;
  ...
  const handleAdd = async () => {
    ...
    } catch { setMsg("Error creating asset"); }
  };

  const handleSaveEdit = async () => {
    if (!eName.trim()) { setMsg("Name required"); return; }
    try {
      const r = await fetch(`${API_URL}/assets/${editAsset.id}`, {
  ...
  const handleAssign = async () => {
    if (!assignJobId) { setMsg("Select a job"); return; }
    try {
      const r = await fetch(`${API_URL}/assets/${assignAsset.id}/assign`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const openEdit = (a) => {
    setEditAsset(a); setEName(a.name); setEType(a.type || ""); setEUnit(a.unit_number || ""); setESerial(a.serial_vin || ""); setENotes(a.notes || ""); setEStatus(a.status);
  };

  const statusBadge = (s) => {
    const cfg = STATUS_COLORS[s] || STATUS_COLORS.available;
    return <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 3, background: cfg.bg, color: cfg.color, letterSpacing: "0.06em", border: `1px solid ${cfg.color}33` }}>{cfg.label}</span>;
  };
    ...
    <div style={{ padding: isMob ? "16px 12px" : "24px 28px" }}>
      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>{counts.available} available · {counts.deployed} deployed · {counts.maintenance} maintenance</div>
        </div>
        <Btn onClick={() => setShowAdd(s => !s)}>{showAdd ? "CANCEL" : "+ ADD ASSET"}</Btn>
      </div>

      {msg && <div style={{ padding: "8px 14px", background: msg.includes("fail") || msg.includes("Error") || msg.includes("required") ? "#fdecea" : "#e6f5ec", borderRadius: 4, fontSize: 12, fontWeight: 700, color: msg.includes("fail") || msg.includes("Error") || msg.includes("required") ? C.red : C.green, marginBottom: 12 }}>{msg}</div>}

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16, alignItems: "center" }}>
        {[["all", `All (${counts.all})`], ["available", `Available (${counts.available})`], ["deployed", `Deployed (${counts.deployed})`], ["maintenance", `Maintenance (${counts.maintenance})`]].map(([k, label]) => (
          <FilterBtn key={k} active={filter === k} onClick={() => setFilter(k)}>{label}</FilterBtn>
        ))}
        <input style={{ ...inputStyle, width: 200, padding: "5px 10px", fontSize: 12 }} placeholder="Search assets..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      {showAdd && (
        <div style={{ background: C.steel, border: `1px solid ${C.border}`, borderRadius: 6, padding: 16, marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: isMob ? "1fr" : "1fr 1fr 1fr 1fr 1fr", gap: 10, marginBottom: 12 }}>
            <div><label style={labelStyle}>NAME *</label><input style={inputStyle} value={newName} onChange={e => setNewName(e.target.value)} placeholder="e.g. Truck #12" /></div>
            ...
            <div><label style={labelStyle}>UNIT #</label><input style={inputStyle} value={newUnit} onChange={e => setNewUnit(e.target.value)} placeholder="Unit number" /></div>
            <div><label style={labelStyle}>SERIAL / VIN</label><input style={inputStyle} value={newSerial} onChange={e => setNewSerial(e.target.value)} placeholder="Optional" /></div>
            <div><label style={labelStyle}>NOTES</label><input style={inputStyle} value={newNotes} onChange={e => setNewNotes(e.target.value)} placeholder="Optional" /></div>
          </div>
      ...
      {!isMob && (
        <div style={{ border: `1px solid ${C.border}`, borderRadius: 6, overflow: "hidden" }}>
          ...
          {filtered.map((a, i) => {
            ...
            return (
              <div key={a.id} style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px 140px 120px 1fr 180px", padding: "10px 16px", alignItems: "center", borderBottom: `1px solid ${C.border}22`, background: i % 2 === 0 ? C.cardBg : C.steel }}>
                ...
                <div style={{ fontSize: 12, color: job ? C.text : C.muted }}>
                  {job ? `#${job.id} — ${job.customer}` : "—"}
                </div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <button onClick={() => openEdit(a)} style={{ background: "transparent", border: `1px solid ${C.blue}44`, color: C.blue, fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 3, cursor: "pointer" }}>EDIT</button>
                  {a.status === "available" && <button onClick={() => { setAssignAsset(a); setAssignJobId(""); }} style={{ background: "transparent", border: `1px solid ${C.green}44`, color: C.green, fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 3, cursor: "pointer" }}>DEPLOY</button>}
                  {a.status === "deployed" && <button onClick={() => handleUnassign(a.id)} style={{ background: "transparent", border: `1px solid ${C.orange}44`, color: C.orange, fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 3, cursor: "pointer" }}>RETURN</button>}
                </div>
              </div>
            );
          })}
        ...
        return (
          <div key={a.id} style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 6, padding: 14, marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {a.status === "deployed" && <button onClick={() => handleUnassign(a.id)} style={{ background: "transparent", border: `1px solid ${C.orange}44`, color: C.orange, fontSize: 11, fontWeight: 700, padding: "5px 12px", borderRadius: 4, cursor: "pointer" }}>RETURN</button>}
            </div>
          </div>
        );
      })}
      ...
      {editAsset && (
        <ModalWrap title={`Edit Asset — ${editAsset.name}`} onClose={() => setEditAsset(null)} width={440}>
          <div style={{ display: "grid", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div><label style={labelStyle}>UNIT #</label><input style={inputStyle} value={eUnit} onChange={e => setEUnit(e.target.value)} /></div>
              <div><label style={labelStyle}>SERIAL / VIN</label><input style={inputStyle} value={eSerial} onChange={e => setESerial(e.target.value)} /></div>
            </div>
            <div><label style={labelStyle}>NOTES</label><input style={inputStyle} value={eNotes} onChange={e => setENotes(e.target.value)} /></div>
          </div>
