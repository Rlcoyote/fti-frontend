  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      ...
      <div style={{ fontSize: 12, color: C.muted, marginBottom: 20 }}>
        {users.length} crew members · {crewData.filter((c) => c.status === "On Job").length} on jobs · {crewData.filter((c) => c.status === "Available").length}{" "}
        available
      </div>
            ...
            {c.currentJob ? (
              <div style={{ background: C.steel, borderRadius: 4, padding: "8px 12px" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
                  Work Order #{c.currentJob.id} — {c.currentJob.customer}
                </div>
